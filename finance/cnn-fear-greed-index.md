...
자주 호출하면 Google Apps Script quota나 CNN 측 차단에 걸릴 수 있으므로, 위 코드는 `CacheService`로 5분 동안 같은 응답을 재사용한다. 단, CNN 원본 JSON에는 긴 historical data가 들어 있어 그대로 캐시하면 `Argument too large: value` 오류가 날 수 있다. 그래서 예제 코드는 현재 요약값과 7개 하위 지표만 남긴 작은 객체를 캐시한다.
